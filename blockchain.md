...
합의를 유지하기 위해서 모든 풀 노드들\(all full nodes\)은 블록들이 같은 합의 규칙\(the same consensus rules\)을 사용하는지 검증해야 한다. 하지만 때때로 새로운 기능\(new features\)을 도입하거나 네트워크 남용\(network abuse\)을 방지\(prevent\)하기 위해 합의 규칙은 변경된다. 
