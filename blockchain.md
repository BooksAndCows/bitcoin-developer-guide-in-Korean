...
모든 블록은 적어도 하나 이상의 트랜잭션을 포함하고 있다. 포함하고 있는 트랜잭션 중 첫 번째 트랜잭션은 반드시 코인베이스 트랜잭션\(coinbase transaction, 생성 트랜잭션\(generation transaction\)라고도 불림\)으로 구성된다. 이 특별한 트랜잭션은 블록 보상을 취합하고 소비할 수 있다 \(블록 생성 보상으로 주어지는 비트코인과 블록 내에 포함하고 있는 트랜잭션으로부터 발생한 수수료들\).

코인베이스 트랜잭션의 UTXO는 특별한 조건을 가지고 있다. 이 UTXO는 적어도 100번의 컨펌을 받아야 한다는 조건이다. 즉 100번째 블록이 추가로 더 생성되기 전에는 코인베이스 트랜잭션의 UTXO는 소비될 수 없다. 이 조건은 일시적으로 마이너가 트랜잭션 수수료와 블록 생성 보상을 소비할 수 없게끔 만든다. 코인베이스가 포함되어 있는 블록이 후에 블록체인 포킹으로 인하여 실효\(효력을 잃은, stale\)블록이 될 수도 있기 때문이다.

블록에는 코인베이스가 아닌 트랜잭션이 포함될 필요는 없지만 마이너는 트랜잭션 수수료를 징수하기 위해 거의 항상 추가적으로 트랜잭션을 포함한다.

코인베이스 트랜잭션을 비롯하여 모든 트랜잭션은 이진 rawtransaction 형식으로 블록에 인코딩 된다.  
이진 rawtransaction 형식은 해쉬되어 트랜잭션 식별자\(txid\)가 된다. 이러한 txids로부터, 머클 트리\(merkle tree\)가 생성된다. 각 txid가 두 개씩 짝지어져 함께 해쉬된다. 만약 txid가 홀수개로 존재하면, 짝이 없는 txid는 자신의 복사\(copy\)본과 함께 해쉬된다.
